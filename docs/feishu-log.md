...
## [2026-09-15] 视觉系统修润：大屏ECharts文字排布修复 + 地图主页圆角卡片容器
- **日期时间**：2026-09-15 21:27
- **操作人**：成员（浅田奈世）
- **模块**：显示层 / 样式规范
- **做了什么修改**：① 修复态势大屏 `BigScreen.vue` 中多维 ECharts 轴标签、图例及标题文字在不同分辨率下的重叠与排布换行问题，完善图表内边距自适应配置；② 针对地图主页 `HomeMap.vue` 与各悬浮抽屉容器，统一遵循齐风鲁韵国风设计系统（4px/2px 结构化微圆角、宣纸墨石底色、去 AI 胶囊感），增强组件视觉呼吸感与空间层级。
- **尝试的实现方法**：ECharts `axisLabel` 增加 `interval: 0`、`rotate` 倾斜或格式化截断换行；容器过渡统一约束为纯透明度平滑淡入 `transition: opacity 200ms ease-out`，杜绝任何形变缩放导致 OpenLayers 视口重绘跳动。
- **遇到的问题**：部分卡片在缩放时仍存在 1px 细微溢出或与地图底图边缘撕裂。
- **解决方案**：统一采用标准设计系统 Token（`--zi-paper`、`--zi-ink`、`--zi-border`），卡片边缘明确 1px 宣纸古铜细描边。
- **创新点**：在不损伤 WebGIS 画布渲染性能的前提下，实现大屏图表与主页卡片的细腻国风排版质感。
- **测试记录**：`npx vue-tsc -b --noEmit` 通过；大屏 1920x1080 及主流笔记本视口下渲染文字排布无重叠，控制台 0 报错。
- **关联提交/文件**：`6b27bdb`、`src/views/BigScreen.vue`、`src/views/HomeMap.vue`

---

## [2026-09-15] 文创订单快递路线追踪：物流状态模拟 + 高德真实驾车路网折线绘制
- **日期时间**：2026-09-15 21:18
- **操作人**：成员（浅田奈世）
- **模块**：显示层 / 逻辑层 / 服务端代理
- **做了什么修改**：① 显示层新增物流跟踪抽屉组件 `src/components/shop/LogisticsTrack.vue`，在「我的订单」列表中点击「查看物流」可滑出专属快递轨迹面板；② 逻辑层新增 `src/services/logistics/logistics.ts`，基于订单发货地（如济南非遗工坊）与用户收货地址（省市区三级联动真实地址），调用高德驾车路线规划 API 绘制真实公路干线路网折线；③ 在轨迹上依据订单状态动态计算快递运输实时节点，并以脉冲标记点实时高亮当前位置，辅以顺丰/中通快递时间轴详情展示。
- **尝试的实现方法**：复用服务端高德驾车路线代理 `/api/amap/direction/driving` 获取精确路网坐标串，通过 `MapAdapter` 动态挂载/卸载独立矢量图层进行轨迹展示与视口自适应缩放（`fitExtent`）。
- **遇到的问题**：订单在「待发货」状态下尚无终点或中转节点，且同城与跨市路线在缩放定位时极值范围差异大。
- **解决方案**：设计防御性模拟逻辑，待发货状态展示仓储出库动效，已发货/运输中状态基于路径百分比插值算出货车坐标；图层缩放添加 40px 内边距 padding 保证轨迹完整可见。
- **创新点**：将传统电商单一的文字时间轴物流，升级为"真实高德路网折线 + 货运节点空间地图映射"的交互式空间物流追踪体验。
- **测试记录**：模拟订单测试查看物流，成功解析路线并在弹出地图上绘制深蓝路网高亮折线；多订单连续切换无图层残留泄漏。
- **关联提交/文件**：`a0c7f1f`、`779ae85`、`src/components/shop/LogisticsTrack.vue`、`src/services/logistics/logistics.ts`、`src/views/MyOrders.vue`

---

## [2026-09-14] 鹰眼图（OverviewMap）控件与全域缩略视口联动
- **日期时间**：2026-09-14 09:05
- **操作人**：成员 2 / mianwu27-ui
- **模块**：显示层 / 地图引擎（OpenLayers）
- **做了什么修改**：① 新增地图鹰眼图（OverviewMap）全域缩略图控件，集成至主地图右下角；② 鹰眼底图加载轻量级山东全省背景图层，并用半透明高亮矩形视框动态同步主视口的缩放级别与平移范围；③ 优化鹰眼图展开/收起切换交互，配备国风边框与微圆角样式。
- **尝试的实现方法**：采用 OpenLayers 原生 `ol/control/OverviewMap` 进行二次封装，定制底图源为静态轻量 OSM/天地图底图，绑定主 Map 实例 View 进行双向同步。
- **遇到的问题**：由于主地图存在侧边抽屉展开（左侧 380px，右侧 420px），鹰眼图原生容器容易被抽屉遮挡或受到 CSS z-index 干扰。
- **解决方案**：通过定制 CSS 定位与 z-index 层级，将鹰眼图锚定在右下角安全显示区域，并在抽屉动画完成时触发 `map.updateSize()` 确保视口矩形精准映射。
- **创新点**：在大幅面探索齐鲁非遗点位时，为用户提供宏观地理全景上下文，极大提升了空间方位感与研学导览沉浸度。
- **测试记录**：主地图缩放平移时，鹰眼矩形无延迟同步联动；展开收起交互流畅，控制台无报错。
- **关联提交/文件**：`9b687e9`、`src/services/map/OLMapAdapter.ts`、`src/components/map/MapContainer.vue`

---

## [2026-09-10] 数据管理模块全面增强：4-Sheet 规范模板下载 + 0~100质量体检评分与等级评定 + CSV体检报告导出 + WMS图层探测
- **日期时间**：2026-09-10 10:36
- **操作人**：成员 6 / lidongtian987-dotcom
- **模块**：后端 / 数据层 / 逻辑层 / 显示层
- **做了什么修改**：① 后端 `server/scripts/data-manage.mjs` 扩展 4-Sheet Excel 模板生成（包含非遗名录、数据字典、填写规范及示例数据 4 个工作表）；② 数据体检引擎升级，增加多维度质量加权计算：综合得分（0~100分）、质量等级（A优/B良/C及格/D较差）、扣分明细与问题要素行号收集；③ 新增 `POST /api/health-check/export-csv` 与 `POST /api/health-check/export`，支持体检诊断报告导出为结构化 CSV/XLSX 文件；④ 服务端集成 OGC WMS 探测代理（`GET /api/wms/capabilities`），前端解析并直接以 OpenLayers ImageWMS 叠加透明图层；⑤ `DataManage.vue` 重构为三栏式专业空间数据工作台（模板与规范 / 上传与格式转换 / 数据体检与服务接入）。
- **尝试的实现方法**：体检扣分规则量化（字段缺失扣分、坐标越界/不在山东省域扣分、重复项扣分）；CSV 导出采用 UTF-8 BOM 防中文乱码；WMS capabilities 正则提取图层 `Name`、`Title`、`BoundingBox`。
- **遇到的问题**：CSV 导出在 Excel 打开时因缺少 BOM 头出现中文乱码；大型 Excel 文件解析耗时导致体检卡顿。
- **解决方案**：在 CSV 输出流前缀 `\uFEFF` 字节序标记；优化后端解析循环，单条数据校验微秒级完成。
- **创新点**：构建了 GIS 行业标准的“数据模板导引 → 规范采集导入 → 自动化空间体检与等级判定 → 诊断报告归档 → OGC服务发现”端到端空间数据资产治理闭环。
- **测试记录**：`npx vue-tsc -b --noEmit` 编译通过；多格式文件上传体检准确输出 85 分/B 级，导出 CSV 包含全部明细且 Excel 正常打开无乱码；WMS 服务地址实测成功探测并叠加图层。
- **关联提交/文件**：`16a63e9`、`8c39302` (PR #36)、`server/scripts/data-manage.mjs`、`server/index.js`、`src/views/DataManage.vue`、`src/services/map/OLMapAdapter.ts`

---

## [2026-09-10] 聚合功能增强 + 地图控件套件 + 用户数据集加载自适应
- **日期时间**：2026-09-10 09:46
- **操作人**：成员 2 / mianwu27-ui
- **模块**：显示层 / 逻辑层 / 地图适配器
- **做了什么修改**：① 新增地图控制套件组件 `src/components/map/MapControls.vue`，集成比例尺（ScaleLine）、指北针方位重置（Reset North）、实时 Zoom 级别展示、重置全省视野视图（Reset Extent）与鼠标实时经纬度坐标（MousePosition）；② 聚合功能深度增强：点击聚合簇不再仅是粗暴放大，而是弹出 `ClusterPopup.vue` 列表弹窗，展示该簇所包含的所有非遗要素清单，点击列表项平滑飞入单项详情并高亮；③ 详情高亮样式升级：增加三层同心圆扩散光晕效果，增强视觉锁定感；④ 用户自定义上传数据集挂载地图后，实现自动计算要素 Extent 范围并平滑飞越适配视野（`fitExtent`）。
- **尝试的实现方法**：利用 OpenLayers `Select` / `pointermove` 与 `getFeatures()` 获取聚合要素簇子集；Popup 挂载为 `ol/Overlay`，支持点击外部或右上角自动关闭。
- **遇到的问题**：高缩放级别下多个点完全重合无法通过点击解聚展开。
- **解决方案**：通过 `ClusterPopup` 浮层展示重合点的列表卡片，支持用户在弹窗中精准点选具体项目并触发飞行动画。
- **创新点**：解决了 WebGIS 点要素重合时的交互死锁痛点；地图控件套件遵循齐风鲁韵国风 Token，彻底摒弃原生粗糙样式。
- **测试记录**：密集点位聚合点击弹出清单，点击子项平滑穿透至详情；缩放平移坐标与比例尺动态刷新，精度达小数点后 4 位。
- **关联提交/文件**：`4e3d7b0`、`dbe06ca`、`35e8784` (PR #34)、`src/components/map/MapControls.vue`、`src/components/map/ClusterPopup.vue`、`src/services/map/OLMapAdapter.ts`
...
## [2026-09-08] 视觉系统工业级规范重构 + 空间分析工作台深度融合
- **日期时间**：2026-09-08 16:45
- **操作人**：全栈（AI 代理）
- **模块**：显示层 / 逻辑层 / 样式规范
- **做了什么修改**：
  1. 彻底清除界面中滥用 `border-radius: 999px` 的 AI 生成特征胶囊与漂浮卡片；
  2. 确立 4px/2px 结构化微圆角设计规范（停靠轨 0px，卡片 4px，标签 2px）；
  3. 顶部导航栏固钉化：重构为 50px 宣纸牙白（`#fffdf8`）与金茶细线（`#e2d6be`）固钉栏，配设古典朱砂红「齐鲁」方印与华文中宋标题；
  4. 布局结构重塑：左侧 48px 墨石色垂直停靠轨 + 380px 停靠抽屉（名录/空间分析/态势/时空演变）+ 右侧 420px 统计透视停靠抽屉；
  5. 将原独立空间分析页面无缝合并融入地图主页左侧工作台抽屉；
  6. 接入高德开放平台省域联想搜索、实况城市天气代理与自驾路网轨迹绘制；
  7. 全站卡片（数据管理、图表研学、自驾路线）几何规范统一收敛为 4px 微圆角。
- **尝试的实现方法**：Headless Chrome CDP 自动化截屏走查校验，验证地图 Canvas 无形变抖动与纯透明度过渡平滑性。
- **遇到的问题**：无节制的圆角胶囊与悬浮卡片造成强烈 AI 样板味与界面漂浮感；全屏形变过渡导致 WebGIS 视口重绘模糊。
- **解决方案**：建立 DESIGN_SPEC 规范；严禁用 `transform: scale`，统一轻量纯透明度淡入；建立 4px/2px 几何秩序。
- **创新点**：以国家数字博物馆（ihchina.cn）与专业 GIS 工业标准（Google Earth / Amap）为标杆，实现古典齐鲁金石美学与 WebGIS 专业性的高度统一。
- **关联提交/文件**：3bfc146、docs/DESIGN_SYSTEM.md、src/styles/theme.css、src/App.vue、src/views/HomeMap.vue

## [2026-09-09] 数据库体系梳理与项目全景文档规范化
- **日期时间**：2026-09-09 22:15
- **操作人**：架构（AI 代理）
- **模块**：后端 / 数据库 / 文档
- **做了什么修改**：
  1. 全面梳理服务端 3 个 SQLite 数据库文件与全部 10 张数据表字典，撰写 `docs/DATABASE.md`；
  2. 整理并新建系统设计规范 `docs/DESIGN_SYSTEM.md`；
  3. 整理并新建系统整体架构与 API 接口字典 `docs/ARCHITECTURE.md`；
  4. 更新工作手册 `docs/HANDBOOK.md`、开发事项总账 `docs/DEV_PLAN.md` 与实习执行报告 `docs/INTERNSHIP_REPORT.md`。
- **尝试的实现方法**：利用 `node:sqlite` 动态读取 sqlite_master 校验全部线上真实表与行数，保证文档与代码 100% 吻合。
- **数据库盘点成果**：
  - `users.db`：users / sessions / addresses（3 张表）
  - `shop.db`：heritage / products / cart / orders / order_items（5 张表）
  - `interact.db`：likes / comments（2 张表）
- **创新点**：代码、数据库、文档、视觉规范全面对齐，零隐式黑盒，便于团队成员与评审专家直接查阅。
- **关联提交/文件**：docs/DATABASE.md、docs/DESIGN_SYSTEM.md、docs/ARCHITECTURE.md、docs/HANDBOOK.md、docs/DEV_PLAN.md

## [里程碑全景总结]
- [x] T1 185项国家级非遗标准数据与传承人入库（WGS84）
- [x] T2 高德矢量/影像 + 天地图 WMTS 多源底图与代理
- [x] T3 前端 OpenLayers 真实瓦片渲染、动态聚类与核密度热力
- [x] T4 数据管理页 + 四大文化走廊专题仓储 + Shp/Excel/GeoJSON 转换体检
- [x] T5 多维属性交集组合筛选 + 高德全省联想搜索
- [x] T6 第一批至第五批时空演变动态播放器与批次统计
- [x] T7 空间分析工具集（测距/测面/缓冲区/叠置统计）合并至地图工作台
- [x] T8 非遗全要素详情页 + 多媒体相册 + 传承人图谱
- [x] T9 多维 ECharts 研学看板 + 地图双向反向联动
- [x] T10 高德公路自驾航迹规划与实况天气 + 12306 高铁路线与经停站规划
- [x] T11 非遗公众点赞口碑 + 研学互动评论（持久化 SQLite）
- [x] T12 全屏数字孪生文化态势展陈大屏（BigScreen）
- [x] T13 非遗 IP 文创衍生商城、购物车、订单事务与管理后台
- [x] T14 个人中心与省市区三级级联地址簿（element-china-area-data）
- [x] T15 视觉系统工业级重构（去 AI 胶囊、50px 固钉顶栏、48px 墨石停靠轨、4px/2px 结构化微圆角）
- [x] T16 数据库全景字典（3 库 10 表）与工程文档体系定稿
