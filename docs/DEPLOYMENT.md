...
### 4.0 完成情况（2026-09 已落地）

以下项**已在代码中实现并实测通过**（提交 `991b512`）：

| 项 | 实现 | 实测结果 |
| :--- | :--- | :--- |
| 上传接口鉴权 | `/api/convert/shp`、`/api/convert/excel` 挂 `requireAuth` | 匿名调用返回 **401 请先登录** |
| 上传体积收紧 | `fileSize 20MB`、`files 5` | 已生效 |
| 请求体上限 | 守卫置于 `express.json` 之前：常规 1MB / 体检 12MB | 2MB body 打常规接口返回 **413 请求体过大** |
| 全站限流 | 内存滑动窗口，单 IP 600 次/分钟 | 80 次瓦片 + 20 次接口请求全部正常，无误伤 |
| 分级限流 | 登录 8 / 注册 3 / 上传 3 / 写操作 30 / 第三方代理 90（次·分钟） | 登录用户第 4 次上传返回 **429** |
| 登录失败锁定 | 同 IP+账号 15 分钟内失败 5 次锁定 | 第 6 次错误密码返回 **429 并提示剩余分钟数**；正常登录不受影响 |
| 会话过期 | `sessions.expires_at`（7 天）+ 鉴权校验 + 定时清理 | 新建会话 `expires_at` 正确落库为 7 天后 |
| 上传文件清理 | 每小时清理超过 24 小时的残留 | 启动即清理了积压的 4 个残留文件 |
| 反向代理信任 | `app.set('trust proxy', 'loopback')` | 保证 nginx 后能取到真实客户端 IP |

**顺带修复的既存 bug**：Excel → GeoJSON 转换原本必然失败 —— `xlsx` 0.18 是 CommonJS 包，
ESM 命名空间里没有 `readFile`（它挂在 `default` 上），而 `upload-utils.mjs` 直接调用了
`XLSX.readFile`。已改为 `mod.default ?? mod`，并用真实 xlsx 做过端到端验证
（2 个有效点转换成功、非法坐标行正确跳过）。

**尚未处理**（留到上线前）：nginx 层限流与安全响应头（P0/P2，属部署配置）、
`xlsx` 依赖替换（M2，需回归测试）、`roads.json` 缓存优化（P2）。
