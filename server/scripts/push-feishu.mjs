...
// 用法: npm run feishu:push   （凭证读根目录 .env）
...
if (!APP_ID || !APP_SECRET) {
  console.error('缺少 FEISHU_APP_ID / FEISHU_APP_SECRET（检查根目录 .env）');
  process.exit(1);
}
...
async function main() {
  ...
  // 1.2 校验已存 folderToken 是否仍可用（可能被清理/失效，直接复用会 403 forbidden）
  let folderToken = state.folderToken || '';
  if (folderToken) {
    const probe = await fetch(`${BASE}/drive/v1/files?folder_token=${folderToken}&page_size=1`, {
      headers: { Authorization: `Bearer ${token}` },
    });
    const probeJ = await probe.json();
    if (probeJ.code !== 0) {
      console.log(`旧文件夹已失效（${probeJ.code}: ${probeJ.msg}），重新创建…`);
      folderToken = '';
    }
  }
  if (!folderToken) {
